...
import live.einfachgustaf.mods.smp.advancement.CompiledAdvancement
...
object MongoDB {
    ...
    fun awardAdvancement(uuid: String, advancement: CompiledAdvancement) {
    ...
    fun revokeAdvancement(uuid: String, advancement: CompiledAdvancement) {
        mcCoroutineScope.launch {
            val advancements = getPlayerAdvancements(uuid).toMutableSet()
            advancements -= advancement
            upsert(uuid, PlayerAdvancementData(uuid, advancements.map { it.id.toString() }.toSet()))
    ...
    suspend fun getPlayerAdvancements(uuid: String): Set<CompiledAdvancement> {
